...
const CartSummary = () => {    
  ...
  return (
    <div className="w-1/4 h-fit p-4 border border-[#064f38] rounded-xl">
        <h2 className="font-semibold text-2xl text-[#064f38] py-2">Summary</h2>
        <div className="border-y border-[#064f38] py-4 space-y-2">
        ...
        <p className="border-b border-[#064f38] py-2 text-lg flex items-center justify-between"><span className="font-semibold">Total: </span> ${(parseInt(getTotalPrice()) + (totalCount > 0 ? 1000.00 : 0.00)).toFixed(2)}</p>

        <div className="my-8">
            <p className='text-theme'>Do you have a discount?</p>
            <div className="w-full text-theme flex items-center gap-2">
                <input type="text" placeholder="discount code" className="w-2/3 border-2 border-[#bbea70] shadow-md placeholder:uppercase rounded-lg py-2 px-1" />
